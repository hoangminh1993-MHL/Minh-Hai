...
lead_clean_data = {
    ...
    "lead-excel-28-503": {
        ...
        "note": "CN : đợi KH xin thông tin NCC về lô hàng gạch\n10/7 : KH đang đợi NCC cập nhật ttin sp"
    },
    "lead-excel-28-399": {
        "name": "Nextstone Vietnam",
        "note": "CN : đợi KH xin thông tin NCC về lô hàng gạch\n10/7 : KH đang đợi NCC cập nhật ttin sp"
    ...
    "lead-excel-19-165": {
        ...
        "note": "[Mã KH: MH409 - Vũ Huyền] Hỏi KG : bánh đậu xanh, ... gửi sang TQ\nĐang tư vấn báo giá"
    },
    "lead-excel-19-647": {
        "name": "Phạm Thị Anh Ngọc",
        "note": "[Mã KH: MH409 - Vũ Huyền] Hỏi KG : bánh đậu xanh, ... gửi sang TQ\nĐang tư vấn báo giá"
...
for lead in db.get("leads", []):
    lid = lead.get("id")
    if lid in lead_clean_data:
        info = lead_clean_data[lid]
        lead["name"] = info["name"]
        lead["note"] = info["note"]
        if "failReason" in info:
            lead["failReason"] = info["failReason"]
        elif "failReason" in lead:
            lead["failReason"] = "Khách hàng ko quan tâm"
        count += 1
...
with open("d:/antigravity/minhhai_crm_deploy/db.json", "w", encoding="utf-8") as f:
    json.dump(db, f, ensure_ascii=False, indent=2)

print(f"Successfully cleaned {count} lead notes and names in db.json to v20.85!")
