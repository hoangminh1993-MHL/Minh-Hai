...
class CleanDbJsonSafely {
    static void Main() {
        string path = @"d:\antigravity\db.json";
        string text = File.ReadAllText(path, Encoding.UTF8);

        // Sanitize all stage names
        ...
        // Set dbVersion to 21.02
        text = Regex.Replace(text, @"""dbVersion"":\s*""\d+\.\d+""", @"""dbVersion"": ""21.02""");

        File.WriteAllText(path, text, new UTF8Encoding(false));
        Console.WriteLine("CleanDbJsonSafely executed successfully!");
    }
}
